...
export default function FacilityChecklistForm() {
  ...
  const getAdvice = async () => {
    ...
    try {
      ...
      const compressedImages = await Promise.all(currentFiles.map(compressImage));
  ...
  if (idx < points.length) {
    ...
    return (
      <div style={{ display: 'flex', minHeight: '100vh' }}>
        <div style={{
          width: '100px', padding: '20px', overflowY: 'auto', position: 'fixed',
          height: '100vh', left: 0, top: 0, boxSizing: 'border-box'
        }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{ background: '#e9ecef', height: '6px', borderRadius: '3px', marginTop: '10px' }}>
              <div style={{ background: '#28a745', height: '100%', borderRadius: '3px', width: `${(Object.keys(formData).filter(k => !['asset_id','asset_manager_name'].includes(k) && !k.includes('_comment')).length / points.length) * 100}%` }} />
            </div>
          </div>
          {points.map((p,i) => {
            const done = Boolean(formData[p.point_id]);
            return (
              <div key={p.point_id} onClick={() => setCurrentIndex(i+1)} style={{
                padding:'12px',marginBottom:'8px',borderRadius:'8px',cursor:'pointer',
                background: done ? '#28a745' : '#fff', color: done ? '#fff' : '#2c3e50',
                border: `1px solid ${done?'#28a745':'#dee2e6'}`
              }}>
                <div style={{ fontWeight:'bold', marginBottom:'4px' }}>
                  {p.libelle}{done && ` (${formData[p.point_id]})`}
                </div>
              </div>
            );
          })}
        ...
        <div style={{
          marginLeft: '100px', padding: '20px', width: 'calc(100% - 100px)',
          ...
          <div className="point-container" style={{ maxWidth: '100%', boxSizing: 'border-box' }}>
              ...
              {currentFiles.length>0 && (
                <div style={{ marginTop:'1rem', display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(150px,1fr))', gap:'10px' }}>
                  {currentFiles.map((file,i)=>(
                    <div key={i} style={{ position:'relative',border:'1px solid #ddd',borderRadius:'8px',overflow:'hidden' }}>
                      <img src={URL.createObjectURL(file)} alt="preview" style={{ width:'100%',height:'120px',objectFit:'cover' }} />
                      <button onClick={()=>removeFile(i)} title="Foto entfernen" style={{ position:'absolute',top:'10px',right:'10px',background:'rgba(255,0,0,0.8)',color:'#fff',border:'none',borderRadius:'50%',width:'25px',height:'25px',fontSize:'14px',cursor:'pointer' }}>×</button>
            ...
            <textarea id="comment" value={comment} onChange={e=>setComment(e.target.value)} placeholder="Kommentar hinzufügen oder KI-Vorschlag bearbeiten..." style={{ width:'100%',minHeight:'100px',marginTop:'0.5rem',padding:'0.8rem',border:'1px solid #1a2a44',borderRadius:'8px',resize:'vertical',borderLeft: comment.includes('Note:')?'4px solid #3b5998':'1px solid #1a2a44' }} />
            <div style={{ marginTop:'1rem' }}>
              <button className="action-button" onClick={()=>{
                if(!selected) { alert('Bitte wählen Sie eine Note aus'); return; }
                setFormData(prev=>({ ...prev, [`${point.point_id}_comment`]:comment }));
                setComment(''); setCurrentFiles([]); if(fileInputRef.current) fileInputRef.current.value='';
                next();
              }}>Weiter ➔</button>
            </div>
          </div>
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh' }}>
      <div style={{ width:'600px',padding:'20px',overflowY:'auto',position:'fixed',height:'100vh',left:0,top:0,boxSizing:'border-box' }}>
        <h3 style={{ marginTop:0, marginBottom:'20px', color:'#2c3e50' }}>Navigation</h3>
        ...
        <div style={{ fontSize:'14px',color:'#6c757d',marginBottom:'20px' }}>Fortschritt: {points.length}/{points.length}</div>
        {points.map(p=>(
          <div key={p.point_id} style={{ padding:'12px',marginBottom:'8px',borderRadius:'8px',cursor:'pointer',background:'#28a745',color:'#fff',border:'1px solid #28a745',fontSize:'13px',lineHeight:'1.3' }}>
            <div style={{ fontWeight:'bold',marginBottom:'4px' }}>{p.libelle} ({formData[p.point_id]})</div>
          </div>
        ))}
      </div>
      <div style={{ marginLeft:'600px',padding:'20px',width:'calc(100% - 600px)',maxHeight:'calc(100vh - 40px)',overflowY:'auto',display:'flex',alignItems:'center',justifyContent:'center',boxSizing:'border-box' }}>
